...
/** Filter to only valid backend enum values, fallback to empty array */
function sanitizeEquipment(list: string[] | undefined): string[] {
...
function sanitizeInjuries(list: string[] | undefined): string[] {
    if (!list || list.length === 0) return [];
    return list.filter(e => (VALID_INJURIES as readonly string[]).includes(e));
}

/** Convert snake_case goal to Title Case */
function humanizeGoal(goal: string | undefined | null): string {
    if (!goal) return '—';
    return goal
        .replace(/_/g, ' ')
...
/** Build user_profile payload for plan generation from stored user data */
function buildProfilePayload(user: FitGenUser | null) {
    if (!user) {
        return {
...
/** Build the full GeneratePlanRequest payload */
function buildPlanRequest(user: FitGenUser | null) {
    return {
